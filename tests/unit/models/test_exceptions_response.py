...
class TestException:
    ...
    def keys_requirement_satisfied(self, response_dict):
        ...
        primary_response_keys = "responseMessage" and "responseCode" and "response" in response_dict.keys()

        if not primary_response_keys:
            return False
        errors_response = response_dict.get("response")["errors"]
        is_list = isinstance(errors_response, list)
        errors_keys_satisfied = "errorCode" and "errorTitle" and "errorDescription" in errors_response[0]
        requirement_clear = is_list and errors_keys_satisfied
        return requirement_clear

    def test_validate_format(self):
        """validation errors format test"""
        validate_response = self.__error_validate_instance.get_response(ErrorManagerTest.PASSWORD_PRECONDITION)
    ...
    def test_auth_format(self):
        ...
        validate_response = self.__error_auth_instance.get_response(ErrorManagerTest.HEADERS_INCORRECT)
        keys_check = self.keys_requirement_satisfied(validate_response[0])
        assert isinstance(validate_response, tuple), "error response should be tuple "
        assert isinstance(validate_response[1], int), "error response invalid ,it should have status code along it"
        assert keys_check, "error response keys are not satisfied"
    ...
    def test_custom_auth(self, status_code, error_description, response_message):
        ...
        assert validate_response[0].get("response")["errors"][0]["errorDescription"] == error_description
