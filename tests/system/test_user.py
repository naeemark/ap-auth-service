...
def test_register_user(setUp_tearDown, setUpClass):
    with setUp_tearDown.get("app")() as c:
        with setUp_tearDown.get("app_context")():
            r = c.post(
                "/register", data={"email": "john12@gmail.com", "password": "123!!@@AB"}
            )
...
def test_password(setUp_tearDown, setUpClass):
    with setUp_tearDown.get("app")() as c:
        with setUp_tearDown.get("app_context")():
            c.post(
                "/register",
                data={"email": "john12@gmail.com", "password": "123!!@@AB123"},
            )
            ...
            auth_request = c.post(
                "/login",
                data=json.dumps(
                    {"email": "john12@gmail.com", "password": "123!!@@AB123"}
                ),
                headers={"Content-Type": "application/json"},
            )
            ...
            password_update_request = c.put(
                "/change-password",
                data=json.dumps({"new_password": "123!!@@AB12"}),
                headers={
...
def test_refresh_token(setUp_tearDown, setUpClass):
    with setUp_tearDown.get("app")() as c:
        with setUp_tearDown.get("app_context")():
            ...
            res_user_login = c.post(
                "/login", data={"email": "john12@gmail.com", "password": "123!!@@AB"}
            )
